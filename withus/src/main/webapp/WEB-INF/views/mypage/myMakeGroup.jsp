	      ...
	      <div class="rightArea" style="flex: 4;">
	         ...
	         <div style="margin-top: 40px;">
	         	<table class="table table-hover">
					...
					<tbody>
					<c:forEach items="${myMakeList}" var="myMakeList" varStatus="loop">
						<tr class="clickable-row"  data-href="" style="text-align: center;">			
							<td>${loop.index + 1}</td>
			                <td><a href='/groups/view/${myMakeList.gno}' class="text-dark"style="text-decoration: none;">${myMakeList.gname}</a></td>
			                <td>${myMakeList.memberid}</td>
			                <td>
							    <c:choose>
							        <c:when test="${myMakeList.catename eq null}">
							            기타
							        </c:when>
							        <c:otherwise>
							            ${myMakeList.catename}
				            ...
				            <td>
							    <c:choose>
							        <c:when test="${myMakeList.rname eq null}">
							            기타
							        </c:when>
							        <c:otherwise>
							            ${myMakeList.rname}
			                ...
			                <td>${myMakeList.gperson}</td>
			                <td>${myMakeList.memberCnt}</td>
			                <td>${myMakeList.gdate}</td>
			                <td>${myMakeList.glike}</td>
			                <td>${myMakeList.greport}</td>					
						</tr>
					</c:forEach>
					</tbody>
				</table>
