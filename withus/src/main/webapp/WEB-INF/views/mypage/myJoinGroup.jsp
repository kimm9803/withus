	      ...
	      <div class="rightArea" style="flex: 4;">
	         ...
	         <div style="margin-top: 40px;">
	         	<table class="table table-hover">
					...
					<tbody>
					<c:forEach items="${myJoinList}" var="myJoinList" varStatus="loop">
						<tr class="clickable-row"  data-href="" style="text-align: center;">			
							<td>${loop.index + 1}</td>
			                <td><a href='/groups/view/${myJoinList.gno}' class="text-dark"style="text-decoration: none;">${myJoinList.gname}</a></td>
			                <td>${myJoinList.memberid}</td>
			                <td>
							    <c:choose>
							        <c:when test="${myJoinList.catename eq null}">
							            기타
							        </c:when>
							        <c:otherwise>
							            ${myJoinList.catename}
				            ...
				            <td>
							    <c:choose>
							        <c:when test="${myJoinList.rname eq null}">
							            기타
							        </c:when>
							        <c:otherwise>
							            ${myJoinList.rname}
			                ...
			                <td>${myJoinList.gperson}</td>
			                <td>${myJoinList.memberCnt}</td>
			                <td>${myJoinList.gdate}</td>
			                <td>${myJoinList.glike}</td>
			                <td>${myJoinList.greport}</td>					
						</tr>
					</c:forEach>
					</tbody>
				</table>
